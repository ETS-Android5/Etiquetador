...
public class Proj4FileReader
{
  ...
  public Proj4FileReader(Context context){
    super();
  ...
  throws IOException 
  {
    ...
    // TODO: parse CSes line-at-a-time (this allows preserving CS param string for later access)
  	//String filename = "./" + authorityCode.toLowerCase();
    //InputStream inStr = Proj4FileReader.class.getClassLoader().getResourceAsStream( filename );
    InputStream inStr = this.context.getAssets().open(authorityCode.toLowerCase());
    if (inStr == null) {
  		throw new IllegalStateException("Unable to access CRS file: " + authorityCode.toLowerCase());
  	}
